...
test.beforeEach(async ({ page }) => {
  ...
  await page.locator("[name=email]").fill("ben@email.com");
...
test("should display hotels", async ({ page }) => {
  ...
  await expect(
    page.getByText("The Taj Mahal was commissioned by Shah Jahan")
  ).toBeVisible();

  await expect(page.getByText("Mumbai, India")).toBeVisible();
  await expect(page.getByText("Family")).toBeVisible();
  await expect(page.getByText("8000 per night")).toBeVisible();
  await expect(page.getByText("85 adults, 2 children")).toBeVisible();
...
test("should edit hotel", async ({ page }) => {
  ...
  await page.locator('[name="name"]').fill("Dombi Dublin Getaways");
  await page.getByRole("button", { name: "Save" }).click();
});
